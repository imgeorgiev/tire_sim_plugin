...
#include "TireFrictionPluginPrivate.h"
#include "TireFrictionPlugin.h"
...
void TireFrictionPlugin::OnUpdate()
{
  ...
  for (int i = 0; i < contacts.contact_size(); ++i)
  {
    ...
    physics::CollisionPtr collPtr1 =
      ...
      this->dataPtr->world->EntityByName(collision1));
    physics::CollisionPtr collPtr2 =
      boost::dynamic_pointer_cast<physics::Collision>(
      this->dataPtr->world->EntityByName(collision2));
    ...
    for (int j = 0; j < contact->position_size(); ++j)
    {
      ...
      {
        ignition::math::Pose3d linkPose = link1->WorldPose();
        ignition::math::Vector3d offset = position - linkPose.Pos();
        velocity1 = link1->WorldLinearVel(offset, ignition::math::Quaterniond());
      }
      {
        ignition::math::Pose3d linkPose = link2->WorldPose();
        ignition::math::Vector3d offset = position - linkPose.Pos();
        velocity2 = link2->WorldLinearVel(offset, ignition::math::Quaterniond());
      }
      ...
      double normalForce;
      {
        ...
        ignition::math::Quaterniond rot1 = link1->WorldPose().Rot();
      ...
      referenceSpeed = std::max(referenceSpeed,
        link1->WorldLinearVel().Length());
      referenceSpeed = std::max(referenceSpeed,
        link2->WorldLinearVel().Length());
